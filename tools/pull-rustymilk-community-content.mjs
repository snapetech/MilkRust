...
const writeManifest = async (pack, destination) => {
  ...
  for (const file of files) {
    ...
    if (relativePath === 'manifest.json' || relativePath === 'MILKRUST-COMMUNITY-NOTICE.md') {
      continue;
    }
    const extension = path.extname(file).toLowerCase();
  ...
  const manifest = {
    ...
    description: 'Community-imported public MilkDrop/projectM content. License status is NOASSERTION; see MILKRUST-COMMUNITY-NOTICE.md.',
    license: 'NOASSERTION',
...
const writeNotice = async (pack, destination, counts) => {
  const byExtension = Object.entries(counts.extensionCounts)
    ...
    .join('\n');
  const notice = `# MilkRust Community Content Notice
...
This directory contains public community MilkDrop/projectM content imported for MilkRust compatibility testing and user-accessible preset libraries.

License status: NOASSERTION. The upstream projectM preset repositories document that many historical MilkDrop presets were freely released but did not carry explicit per-author licenses. MilkRust keeps this content separate from vetted redistributable third-party content.

Default build status: excluded unless a MilkRust distribution explicitly opts into community-unlicensed content.

Removal/contact policy: if you are an original preset, texture, or asset author and want your work removed from MilkRust-hosted community content, open an issue or contact the MilkRust maintainers with the file path(s). The affected files will be removed from future releases.
...
${byExtension || '- none'}
`;
  await fs.writeFile(path.join(destination, 'MILKRUST-COMMUNITY-NOTICE.md'), notice);
};
