...
MilkRust is split into layers so hosts can use only what they need.
...
- WASM: `wasm-bindgen` exports, WebGL2/canvas renderers, browser texture plumbing, and the public `MilkRustEngine` class.
...
- Plugin APIs: JSON/JS plugin descriptors loaded from pack manifests and executed by the web client lifecycle hooks; native Rust hosts also expose `DesktopPlugin` hook points through `milkrust-desktop`.
...
- Desktop host primitives: `milkrust-desktop` crate with deterministic frame/runtime session
...
The extraction target is for slskR to depend on MilkRust instead of carrying the engine implementation inline.
