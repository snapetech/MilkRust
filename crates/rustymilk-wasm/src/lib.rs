...
use rustymilk_core::{
    ...
    serialize_rustymilk_preset_set, validate_rustymilk_import, RustyMilkFrameSetRuntime,
    RustyMilkIndexedEntry, RustyMilkInputState, RustyMilkValue,
};
...
impl WasmRustyMilkEngine {
    ...
    pub fn new(canvas: web_sys::HtmlCanvasElement) -> Result<WasmRustyMilkEngine, JsValue> {
        let source = rustymilk_core::RUSTYMILK_PRESETS
            .first()
            .copied()
            .unwrap_or_default()
            .to_string();
...
impl WasmRustyMilkEngine {
    fn parsed_source(&self) -> rustymilk_core::RustyMilkPresetSet {
