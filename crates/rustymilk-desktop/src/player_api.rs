...
use rustymilk_core::RustyMilkFrameSet;

use crate::{
    build_audio_profile, collect_preset_inputs, report_frame_set, DesktopAudioProfile,
    PackPluginInput, PresetInput, RustyMilkFrameSetReport, RustyMilkFrameSetRuntimeHost,
};
...
pub struct DesktopPlayerFrame {
    ...
    pub source_path: String,
    pub frame_set: RustyMilkFrameSet,
    pub report: RustyMilkFrameSetReport,
}
...
pub struct DesktopPlayerEngine {
    ...
    host: RustyMilkFrameSetRuntimeHost,
...
impl DesktopPlayerEngine {
    ...
    fn new_with_loaded_presets(
        ...
        Ok(Self {
            ...
            host: RustyMilkFrameSetRuntimeHost::new(),
            preset_index: 0,
            local_frame_index: 0,
    ...
    pub fn reset(&mut self) {
        ...
        self.preset_load_pending = true;
        self.host = RustyMilkFrameSetRuntimeHost::new();
    }
...
mod tests {
    ...
    fn resolves_preset_indices_and_advances_playhead() {
        ...
        let nanos = SystemTime::now()
            ...
            .unwrap_or(0);
        let base =
            std::env::temp_dir().join(format!("rustymilk-desktop-player-api-{process_id}-{nanos}"));
    ...
    fn supports_audio_provider_override() {
        ...
        let base = std::env::temp_dir().join(format!(
            "rustymilk-desktop-player-audio-provider-{process_id}-{nanos}"
        ));
        std::fs::create_dir_all(&base).unwrap();
    ...
    fn plugin_hooks_mutate_preset_source_and_see_frame_events() {
        ...
        let nanos = SystemTime::now()
            ...
            .unwrap_or(0);
        let base = std::env::temp_dir().join(format!(
            "rustymilk-desktop-player-plugin-hooks-{process_id}-{nanos}"
        ));
        std::fs::create_dir_all(&base).unwrap();
