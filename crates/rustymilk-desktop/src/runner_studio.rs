...
use rustymilk_core::{
    ...
    validate_rustymilk_import, RustyMilkCompatibilityEntry, RustyMilkCompatibilitySummary,
};
...
struct StudioPresetRecord {
    ...
    compatibility_entry: Option<RustyMilkCompatibilityEntry>,
}
...
pub fn run_desktop_studio(args: &[String]) -> i32 {
    ...
    for preset in &selected_presets {
        ...
        let validation_title = match validate_rustymilk_import(&source) {
            Ok(title) => Some(title),
            Err(error) => {
                reports.push(StudioPresetRecord {
                    path: preset.source_path.to_string_lossy().to_string(),
    ...
    let compatibility_entries = reports
        ...
        .collect::<Vec<_>>();
    let compatibility_summary = summarize_rustymilk_compatibility_matrix(&compatibility_entries);
...
fn print_studio_help() {
    println!("rustymilk-desktop studio - compatibility inspection and preset report mode");
    println!();
    println!("Usage:");
    println!("  cargo run -p rustymilk-desktop --bin studio -- [options]");
    println!();
    println!("Options:");
...
fn print_studio_text(
    ...
    summary: &RustyMilkCompatibilitySummary,
    pack_plugins: &[crate::PackPluginInput],
) {
    println!(
        "RustyMilk desktop studio: {} entries ({} supported, {} unsupported)",
        reports.len(),
