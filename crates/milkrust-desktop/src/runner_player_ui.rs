...
use milkrust_core::{MilkRustFrameSet, MilkRustPrimitiveMode, MilkRustTexturedPrimitiveMode};
...
use crate::{
    ...
    DesktopPlayerError, MilkRustFrameSetReport,
};
...
pub fn run_desktop_player_ui(args: &[String]) -> i32 {
    ...
    println!(
        "MilkRust desktop UI playback complete: rendered {} frames across {} preset(s).",
...
fn print_player_ui_help() {
    println!("milkrust-desktop player-ui - lightweight desktop windowed mode");
    println!();
    println!("Usage:");
    println!("  cargo run -p milkrust-desktop --features ui --bin player-ui -- [options]");
    println!();
    println!("Options:");
...
mod tests {
    ...
    fn player_ui_start_preset_lookup_delegates_to_engine_api() {
        ...
        let base = std::env::temp_dir().join(format!(
            "milkrust-desktop-ui-api-test-{process_id}-{nanos}"
        ));
