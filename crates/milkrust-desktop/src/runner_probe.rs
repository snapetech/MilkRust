...
fn print_probe_help() {
    println!("milkrust-desktop - prototype MilkRust headless desktop session runner");
    println!();
    println!("Usage:");
    println!("  cargo run -p milkrust-desktop --bin milkrust-desktop -- [options]");
    println!();
    println!("Options:");
    ...
    println!("  cargo run -p milkrust-desktop --bin milkrust-desktop -- --pack examples/sample-pack --frames 24 --json");
    println!();
    println!("Subcommands:");
    println!("  cargo run -p milkrust-desktop --bin player  for player-mode simulation output");
    println!(
        "  cargo run -p milkrust-desktop --bin studio  for studio-style compatibility inspection"
    );
}
...
fn print_probe_text(
    ...
    println!(
        "MilkRust desktop probe: {} presets, {} frames each @ {fps} fps",
        session_reports.len(),
        session_reports
            .first()
