...
fn print_player_help() {
    println!("milkrust-desktop player - session-oriented desktop runtime probe");
    println!();
    println!("Usage:");
    println!("  cargo run -p milkrust-desktop --bin player -- [options]");
    println!();
    println!("Options:");
...
fn print_player_text(
    ...
    println!(
        "MilkRust desktop player: {} presets, {} frames each @ {fps} fps",
        playlist.len(),
        frames
    );
...
fn print_player_json(
    presets_summary: &[(&PresetInput, Vec<crate::DesktopSessionFrame>)],
    playlist: &[PlayerReport],
    compatibility_inputs: &[milkrust_core::MilkRustCompatibilityEntry],
    frames: usize,
    fps: f64,
    frame_log_every: usize,
    include_frame_records: bool,
    ...
    let compatibility = summarize_milkrust_compatibility_matrix(compatibility_inputs);
