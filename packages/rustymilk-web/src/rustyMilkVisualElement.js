import { createRustyMilkEngine } from './rustyMilkEngine.js';
...
export const createRustyMilkVisualizerElement = ({
  tagName = 'rustymilk-visualizer',
  ...
  if (!documentRef || typeof documentRef.createElement !== 'function') {
    throw new Error('rustyMilkVisualElement requires a DOM document context');
  }
  const scheduler = toFrameScheduler();

  class RustyMilkVisualizerElement extends (baseElement || class {}) {
    ...
    _setupCanvas() {
      ...
      if (!this.#canvas.className) {
        this.#canvas.className = 'rustymilk-visualizer-canvas';
      }
    ...
    attributeChangedCallback(name, _oldValue, newValue) {
      if (name === 'module-path') {
        this.#modulePath = newValue || '/pkg/rustymilk_wasm.js';
        return;
      }
      if (name === 'preset-text') {
    ...
    set modulePath(nextPath) {
      this.#modulePath = String(nextPath || '/pkg/rustymilk_wasm.js');
    ...
    async start() {
      ...
      try {
        ...
        this.dispatchEvent(new CustomEvent('rustymilk:ready'));
        this.#loop();
      } catch (error) {
    ...
    stop() {
      ...
      this.dispatchEvent(new CustomEvent('rustymilk:stopped'));
    }
    ...
    #setStatus(status, error = '') {
      ...
      this.dispatchEvent(new CustomEvent('rustymilk:status', { detail: { status, error } }));
    }
  }

  if (customElements && typeof customElements.get === 'function' && !customElements.get(tagName)) {
    customElements.define(tagName, RustyMilkVisualizerElement);
  }

  return RustyMilkVisualizerElement;
};

export const defineRustyMilkVisualizerElement = (options = {}) => {
  const ElementClass = createRustyMilkVisualizerElement(options);
  return ElementClass;
};
