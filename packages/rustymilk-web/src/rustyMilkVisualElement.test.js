...
import {
  createRustyMilkVisualizerElement,
} from './rustyMilkVisualElement.js';
...
describe('createRustyMilkVisualizerElement', () => {
  it('defines and creates a connected visualizer element', async () => {
    ...
    const Element = createRustyMilkVisualizerElement({
      tagName: 'rustymilk-visualizer-test',
      createEngine,
    ...
    assert.equal(restoreElements.get('rustymilk-visualizer-test'), Element);
  ...
  it('supports auto-start behavior through attribute/property', async () => {
    ...
    const Element = createRustyMilkVisualizerElement({
      tagName: 'rustymilk-visualizer-autostart',
      createEngine,
  ...
  it('queues preset and configuration before engine creation', async () => {
    ...
    const fakeEngine = createFakeEngine();
    const createEngine = async () => fakeEngine.methods;
    const element = new (createRustyMilkVisualizerElement({
      tagName: 'rustymilk-visualizer-preset-queue',
      createEngine,
      baseElement: createFakeElementBase(),
  ...
  it('dispatches web component status events and stops rendering on stop()', async () => {
    ...
    const element = new (createRustyMilkVisualizerElement({
      tagName: 'rustymilk-visualizer-events',
      createEngine,
      baseElement: fakeElementBaseClass,
    ...
    const events = [];
    element.addEventListener('rustymilk:ready', () => events.push('ready'));
    element.addEventListener('rustymilk:status', (event) => events.push(`status:${event.detail.status}`));
    await element.start();
  ...
  it('applies status listener callbacks when status changes', async () => {
    ...
    const element = new (createRustyMilkVisualizerElement({
      tagName: 'rustymilk-visualizer-callbacks',
      createEngine,
      baseElement: createFakeElementBase(),
      customElements: createFakeCustomElements(),
      documentRef: fakeDocument,
