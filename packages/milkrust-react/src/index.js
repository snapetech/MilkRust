const createMilkRustBindings = async () => {
  try {
    return await import('@milkrust/web');
  } catch (importError) {
    const fallbackModule = await import('../../milkrust-web/src/milkrustEngine.js');
    return fallbackModule;
  }
};

const webBindings = await createMilkRustBindings();
...
const resolveReact = (react = globalThis.React) => {
  ...
  if (missing.length) {
    throw new Error(
      `createMilkRustReactBindings is missing required React hooks: ${missing.join(', ')}`,
    );
  }
  return react;
};
...
export const createMilkRustReactBindings = (
  react,
  {
    createEngine = createMilkRustEngine,
    packLoader = loadMilkRustPack,
  } = {},
) => {
  ...
  const useMilkRustEngine = ({
    ...
    presetText,
    presetFileName = 'milkrust-react.milk',
    textureAssets = {},
    automation,
    onFrame,
    onError,
  ...
  const useMilkRustPack = (packUrl, { fetchImpl = globalThis.fetch } = {}) => {
  ...
  const defaultCanvasProps = {
    role: 'img',
    'aria-label': 'MilkRust visualizer canvas',
  };
  const MilkRustCanvas = React.forwardRef ? React.forwardRef((props, ref) => {
...
export const createMilkRustReact = createMilkRustReactBindings;
