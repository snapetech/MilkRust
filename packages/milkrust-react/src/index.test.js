...
describe('createMilkRustReactBindings', () => {
  ...
  it('builds reusable React bindings with default hook contract', async () => {
    ...
    const { useMilkRustEngine, MilkRustCanvas, useMilkRustPack } =
      createMilkRustReactBindings(fakeReact, {
        createEngine: fakeCreateEngine,
        packLoader: async () => ({ status: 'ok', plugins: [], presets: [], manifest: {} }),
      });

    const hookState = useMilkRustEngine({
    ...
    assert.equal(typeof MilkRustCanvas, 'function');
    hookState.dispose();

    fakeReact.cleanups.length = 0;

    const packState = useMilkRustPack('http://localhost/test', {
